...
class SystemicVascularResistance:
    def __init__(self, vf: vitaldb.VitalFile):
        ...
        mean_track = next(
            ...
            next((t for t in available_tracks if 'Intellivue/NIBP_MEAN' in t), None)) # Then try for non-invasive mean BP
        
        self.vf = ((vf.to_numpy(track_names = mean_track) - vf.to_numpy(track_names = 'Intellivue/CVP_MEAN')) * 80) / CardiacOutput(vf).values
